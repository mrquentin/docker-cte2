...
if ! [[ -f "Server-Files-$PROJECT_VERSION.zip" ]]; then
  ...
  serverFileName="${fileName%-*} SERVER-${fileName##*-}"
  curl -Lo "Server-Files-$PROJECT_VERSION.zip" "https://edge.forgecdn.net/files/${serverPackFileId:0:4}/${serverPackFileId:4}/${serverFileName// /%20}" || exit 9
